...
from village_items import Whiskey, Elixir, Fruit, Meat, Medicine, Torch, Rope
import time
...
class Player:
    ...
    def __init__(self, name):
        ...
        self.sight = [0, 1]
        self.level = 1
        self.weapons = [["sword", 6, 11, 3]]
        self.spells = [["firebolt", 8, 13, 1]]
    ...
    def deal_dmg(self, enemy, attack_type):
        if not self.alive:
            return
    ...
    def consume_food(self, food):
        if food == "whiskey".lower():
            ...
            self.energy += int(food.energy_bonus * 1.2) if "soldier" in self.companions else food.energy_bonus
            if self.energy > 100:
                self.energy = 100
            ...
            food = Medicine(1)
            self.inventory.remove_item(food)
            self.health_point += int(food.energy_bonus * 1.2) if "shaman" in self.companions else food.energy_bonus
            ...
            food = Elixir(1)
            self.inventory.remove_item(food)
            self.mana_point += food.energy_bonus
    ...
    def injury(self):
        if random.randint(0, 100) <= 50:
            companion = random.choice(list(self.injured_companions.keys()))
            messagebox.showinfo("ouch", "Your " + companion + " can't keep up because of his injuries. "
                                                              "He leaves your group!")
