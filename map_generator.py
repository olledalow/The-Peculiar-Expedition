import random
from terrains import Sea, Ship, Village, Lake, Pyramid, Mountain, Meadow, Jungle, Thicket, Altar, Sanctuary, Cave
...
def reset_map():
    global map_coordinates, map_copy, village_locations, lake_locations, pyramid_position, mountain_locations, \
        altar_locations
    map_coordinates = []  # contains the integer coordinates of the map elements in lists. : [row_index, column index]
    map_copy = []  # copy of map_coordinates
    village_locations = []  # contains the integer coordinates of the village (F) elements in lists. :
...
def remove_fog():
    for num1 in player.sight:
        for num2 in player.sight:
            ...
            try:
                current_terrain = game_map[player.position[0] + num1][player.position[1] + num2]
                current_terrain.actual_image = current_terrain.image
                current_terrain.label.config(image=current_terrain.actual_image)
            except IndexError:
                pass

            try:
                current_terrain = game_map[player.position[0] + num1][abs(player.position[1] - num2)]
                current_terrain.actual_image = current_terrain.image
                current_terrain.label.config(image=current_terrain.actual_image)
            except IndexError:
                pass
            ...
            try:
                current_terrain = game_map[abs(player.position[0] - num1)][player.position[1] + num2]
                current_terrain.actual_image = current_terrain.image
                current_terrain.label.config(image=current_terrain.actual_image)
            except IndexError:
                pass
            ...
            try:
                current_terrain = game_map[abs(player.position[0] - num1)][abs(player.position[1] - num2)]
                current_terrain.actual_image = current_terrain.image
                current_terrain.label.config(image=current_terrain.actual_image)
            except IndexError:
                pass
